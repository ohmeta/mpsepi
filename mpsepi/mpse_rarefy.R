...
Usage:
  mpse_rarefy.R rarefy <mpse> <chunks> <mpse_rarefied_output>
...
mpse_rarefy <- function(mpse, chunks) {
  mpse_rarefied <- 
    mpse %>% 
      MicrobiotaProcess:mp_rrarefy() %>% 
      MicrobiotaProcess:mp_cal_rarecurve(
        .abundance = RareAbundance, 
        chunks = chunks, action="add")
  return(mpse_rarefied)
}
...
if (args$rarefy) {
  ...
  mpse_rarefied <- mpse_rarefy(mpse, args$chunks)

  saveRDS(mpse_rarefied, args$mpse_rarefied_output)
}
